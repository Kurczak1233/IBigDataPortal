import { FileVm } from "interfaces/Models/FilesMetadata/ViewModels/FileVm";
import { FileWithMetadata } from "interfaces/Models/FilesMetadata/ViewModels/FileWithMetadata";
import { JobOfferViewModel } from "interfaces/Models/JobOffers/ViewModels/JobOfferViewModel";
import { useCallback, useEffect, useState } from "react";
import { useLocation } from "react-router-dom";

const EditJobOfferPageLogic = () => {
  const [jobOfferFiles, setJobOfferFiles] = useState<FileWithMetadata[]>([]);
  const location = useLocation();
  const state = location.state as JobOfferViewModel;

  const handleMapFilesVmToFiles = (files: FileVm[]) => {
    return files.map((item) => {
      const file = new File(
        [`data:image/png;base64,${item.base64FileString}`],
        `${item.fileName}`,
        {
          type: item.fileType,
        }
      );
      return { guid: item.guid, file: file };
    });
  };

  const handleFileFiles = useCallback((files: FileVm[]) => {
    setJobOfferFiles(handleMapFilesVmToFiles(files));
  }, []);

  useEffect(() => {
    handleFileFiles(state.files);
  }, [handleFileFiles, state]);

  useEffect(() => {
    if (state) {
      handleMapFilesVmToFiles(state.files);
    }
  }, [state]);

  return { state, jobOfferFiles, setJobOfferFiles };
};

export default EditJobOfferPageLogic;
